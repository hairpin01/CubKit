...
def lint_project(
    ...
    *,
    ...
    if _rule_enabled("locales", manifest.lint):
        issues.extend(_lint_locales(manifest, string_accesses))
...
def _lint_locales(manifest: "Manifest", accesses: list[tuple[Path, int, str, set[str]]]) -> list[LintIssue]:
    if not accesses:
        return []
    if manifest.locales is None:
        return [LintIssue(line, f'locale key "{key}" is used but no locales directory is configured', severity="warning", code="locale-key", path=path) for path, line, key, _ in accesses]
